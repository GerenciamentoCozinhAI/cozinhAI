...
const RecipePage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <p className="text-gray-600 mb-6 text-lg leading-relaxed">{recipe.description || "Sem descrição disponível."}</p>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 p-4 bg-green-50 rounded-lg">
        <div className="flex items-center gap-2 text-gray-700">
          <Clock size={20} className="text-green-600" />
          <span>{recipe.prepTime ? `${recipe.prepTime} minutos` : "Tempo não informado"}</span>
        </div>
        <div className="flex items-center gap-2 text-gray-700">
          <Utensils size={20} className="text-green-600" />
          <span>{recipe.servings ? `${recipe.servings} porções` : "Porções não informadas"}</span>
        </div>
        <div className="flex items-center gap-2 text-gray-700">
      ...
      {recipe.ingredients && recipe.ingredients.length > 0 && (
        <div className="mb-8">
          <h2 className="text-2xl font-semibold text-green-700 mb-4 pb-2 border-b border-green-100">Ingredientes</h2>
      ...
      {recipe.instructions && (
        <div className="mb-8">
          <h2 className="text-2xl font-semibold text-green-700 mb-4 pb-2 border-b border-green-100">Instruções</h2>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mt-8">
        ...
        >
          <Star size={20} className={isFavorited ? "fill-white" : "fill-none"} />
